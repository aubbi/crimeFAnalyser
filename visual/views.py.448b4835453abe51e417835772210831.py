...
def filterData(request):
    ...
    rawData = Crime.objects.filter(
        Date__range=[startDate, endDate], Arrest__in=arrest, Primary_Type__in=types)
    result = []
...
def defaultHome(request):
    ...
    rawData = Crime.objects.filter(
        Date__range=['2016-01-01', '2016-12-31']).order_by('Date')[:50000]
    result = []
    for i in rawData.values():
        i['Date'] = i['Date'].strftime('%m/%d/%Y')
        result.append(i)
